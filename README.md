...
Codex CLI (TUI) 用のステータスライン設定は別リポジトリ [codex-statusline](https://github.com/YATA-NODE/codex-statusline) にあります。あちらは外部スクリプト不要で、Codex 組み込みの設定（`config.toml`）だけで完結します。

本リポジトリは Claude Code 用に Python スクリプトで描画する方式（補助機能として Codex CLI のレート残量を並置表示することも可能）で、両者は対象ツールと仕組みが異なります。
...
The status line setup for the Codex CLI (TUI) lives in a separate repository, [codex-statusline](https://github.com/YATA-NODE/codex-statusline). That one needs no external script — it is fully self-contained using the Codex CLI's built-in settings (`config.toml`).

This repository renders for Claude Code via a Python script (and can optionally show the Codex CLI rate limits alongside as an auxiliary feature); the two differ in their target tool and mechanism.
